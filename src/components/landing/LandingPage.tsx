...
export const LandingPage: React.FC<LandingPageProps> = ({ onNavigate }) => {
  ...
  return (
    <main className="flex-1 bg-white flex flex-col justify-start select-none">
      ...
      {/* =========================================================================
          1. HERO SECTION (With direct drag & drop file / link import box)
          ========================================================================= */}
      <section className="pt-12 sm:pt-14 pb-14 sm:pb-18 border-b border-[#E2E8F0] relative bg-white overflow-hidden">
        <div className="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col items-center space-y-4 text-center">
            ...
            <div id="import-section" className="w-full pt-2">
      ...
      <section className="py-16 sm:py-20 bg-white border-b border-[#E2E8F0]">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center">
            ...
            {/* Right Column: Supported Formats & Platforms Visual Card */}
            <div className="lg:col-span-6">
              <div className="bg-white border border-[#E2E8F0] rounded-xl p-5 sm:p-6 shadow-sm text-left space-y-4">
                <div className="text-xs font-bold text-[#111827] uppercase tracking-wider font-mono">
                  Supported Sources &amp; Formats
                </div>

                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  <div className="p-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-center space-y-1">
                    <span className="text-xs font-bold text-[#111827] block">YouTube</span>
                    <span className="text-[10px] text-[#64748B] block font-mono">Public URLs</span>
                  </div>
                  <div className="p-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-center space-y-1">
                    <span className="text-xs font-bold text-[#111827] block">Zoom</span>
                    <span className="text-[10px] text-[#64748B] block font-mono">Cloud Recordings</span>
                  </div>
                  <div className="p-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-center space-y-1">
                    <span className="text-xs font-bold text-[#111827] block">Google Meet</span>
                    <span className="text-[10px] text-[#64748B] block font-mono">Drive Files</span>
                  </div>
                  <div className="p-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-center space-y-1">
                    <span className="text-xs font-bold text-[#111827] block">MS Teams</span>
                    <span className="text-[10px] text-[#64748B] block font-mono">MP4 / M4A</span>
                  </div>
                  <div className="p-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-center space-y-1">
                    <span className="text-xs font-bold text-[#111827] block">Local Video</span>
                    <span className="text-[10px] text-[#64748B] block font-mono">MP4, MOV, AVI</span>
                  </div>
                  <div className="p-3 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg text-center space-y-1">
                    <span className="text-xs font-bold text-[#111827] block">Local Audio</span>
                    <span className="text-[10px] text-[#64748B] block font-mono">MP3, WAV, AAC</span>
                  </div>
                </div>

                <div className="pt-2 border-t border-[#F1F5F9] flex items-center justify-between text-xs text-[#64748B]">
                  <span>Export directly to:</span>
                  <div className="flex items-center gap-2 font-mono text-[11px] text-[#2563EB] font-semibold">
                    <span>TXT</span>
                    <span>·</span>
                    <span>SRT</span>
                    <span>·</span>
      ...
      <section className="py-16 sm:py-20 bg-[#F8FAFC] border-b border-[#E2E8F0]">
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14 items-center">
            ...
            {/* Left Column: Refined Clean Single Workspace Card Mockup */}
            <div className="lg:col-span-6 order-2 lg:order-1">
              <div className="bg-white border border-[#E2E8F0] rounded-xl p-4 sm:p-5 shadow-sm text-left select-none">
                
                {/* Card Header */}
                <div className="flex items-center justify-between border-b border-[#F1F5F9] pb-3 mb-4">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-[#111827] tracking-tight font-sans">Veyra</span>
                    <span className="text-[#CBD5E1]">·</span>
                    <span className="text-xs font-mono text-[#64748B]">interview.mp4</span>
                  </div>
                ...
                {/* TOP: Compact Realistic Video Preview */}
                <div className="rounded-lg overflow-hidden border border-[#E2E8F0] relative aspect-[21/9] bg-[#0F172A] mb-4">
                  <img
                    src="https://images.unsplash.com/photo-1517245386807-bb43f82c33c4?auto=format&fit=crop&w=800&q=80"
                    alt="Interview recording session"
                    className="w-full h-full object-cover opacity-90"
                    referrerPolicy="no-referrer"
                  />
                  {/* Minimal Video Controls */}
                  <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent px-3 py-2 flex items-center gap-2.5">
                    <button className="text-white hover:text-slate-200 transition-colors cursor-pointer">
                      <Play className="w-3 h-3 fill-current" />
                    </button>
                    <div className="flex-1 h-1 bg-white/30 rounded-full overflow-hidden">
                      <div className="w-[30%] h-full bg-[#2563EB]"></div>
                    </div>
                    <span className="text-[10px] font-mono text-white/90">00:14 / 12:45</span>
                  </div>
                </div>

                {/* BOTTOM: Large Transcript Editor (Primary Focus) */}
                <div className="space-y-3 mb-4">
                  
                  {/* Speaker 1 */}
                  <div className="p-3.5 bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg space-y-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-semibold text-[#0F172A]">Speaker 1</span>
                      <span className="font-mono text-[11px] text-[#94A3B8]">00:05</span>
                    </div>
                    <p className="text-xs sm:text-sm text-[#334155] leading-relaxed">
                      "Welcome everyone. Today we're going to discuss the latest project updates."
                    </p>
                  </div>

                  {/* Speaker 2 (Highlight "customer experience" in Veyra blue with subtle cursor) */}
                  <div className="p-3.5 bg-white border border-[#BFDBFE] rounded-lg shadow-2xs space-y-1 relative ring-1 ring-[#2563EB]/10">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-semibold text-[#2563EB]">Speaker 2</span>
                      <span className="font-mono text-[11px] text-[#2563EB] font-medium">00:14</span>
                    </div>
                    <p className="text-xs sm:text-sm text-[#0F172A] leading-relaxed">
                      "The main focus this quarter is improving{' '}
                      <span className="bg-[#EFF6FF] text-[#2563EB] border border-[#BFDBFE] px-1 py-0.5 rounded font-medium inline-flex items-center">
                        customer experience
                        <span className="ml-0.5 w-[1.5px] h-3.5 bg-[#2563EB] animate-pulse inline-block"></span>
                      </span>
                      ."
                    </p>
                  </div>
                ...
                <div className="pt-3 border-t border-[#F1F5F9] flex items-center justify-between text-xs text-[#64748B]">
                  <div className="flex items-center gap-3 font-mono text-[11px] font-medium text-[#475569]">
                    <span>TXT</span>
                    <span>SRT</span>
                    <span>VTT</span>
                  </div>
                  <span className="text-[11px] font-mono text-[#64748B]">99.4% accuracy</span>
                </div>
